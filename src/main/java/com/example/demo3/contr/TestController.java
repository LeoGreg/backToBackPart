...
public class TestController {
    ...
    @PostConstruct
    private void init() {
        String base64 = Base64.encodeBase64String("interconnect_user:interconnect_password".getBytes());
    ...
    public ResponseEntity getMovies() {
        ResponseEntity<List> movies = restTemplate.exchange("http://localhost:8080/interconnect/getMovies", HttpMethod.GET, new HttpEntity<>(httpHeaders), List.class);
        return movies;
    }
...
//    }
    ...
    public ResponseEntity addM() {
        ...
        try {
            ...
            ResponseEntity<Movie> responseEntity = restTemplate.exchange("http://localhost:8080/interconnect/add", HttpMethod.POST, new HttpEntity<>(movie, httpHeaders), Movie.class);
            return responseEntity;
        } catch (RestClientResponseException exception) {
            log.error(">>>" + exception.getMessage() + ":");
            return ResponseEntity
